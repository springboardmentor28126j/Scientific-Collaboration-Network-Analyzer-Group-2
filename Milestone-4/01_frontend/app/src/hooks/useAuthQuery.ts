...
export function useLogout() {
  ...
  return useMutation({
    mutationFn: async () => {
      sessionStorage.setItem("clear-login-form", "true");
...
export function useInstitutions(options?: { enabled?: boolean }) {
...
export function useInstitutionUsers(role?: "RESEARCHER" | "REVIEWER", options?: { enabled?: boolean }) {
...
export function useUpdateInstitutionUser() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ userId, data }: { userId: string; data: { full_name?: string; description?: string | null } }) => apiClient.updateInstitutionUser(userId, data),
    onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ["institution-users"] }); toast.success("Researcher updated"); },
    onError: (error: Error) => toast.error(error.message || "Failed to update researcher"),
  });
}

export function useDeleteInstitutionUser() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (userId: string) => apiClient.deleteInstitutionUser(userId),
    onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ["institution-users"] }); toast.success("Researcher deleted"); },
    onError: (error: Error) => toast.error(error.message || "Failed to delete researcher"),
  });
}
