...
from app.schemas.institution import InstitutionCreate, InstitutionRead, InstitutionRegister, InstitutionUpdate
...
@router.post("", response_model=InstitutionRead, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_superuser)])
async def create_institution(payload: InstitutionCreate, institution_service: InstitutionService = Depends(get_institution_service)):
    return await institution_service.create_platform_institution(payload)

@router.get("/{institution_id}", response_model=InstitutionRead, dependencies=[Depends(require_superuser)])
async def get_institution(institution_id: uuid.UUID, institution_service: InstitutionService = Depends(get_institution_service)):
    return await institution_service.get_institution(institution_id)

@router.patch("/{institution_id}", response_model=InstitutionRead, dependencies=[Depends(require_superuser)])
async def update_institution(institution_id: uuid.UUID, payload: InstitutionUpdate, institution_service: InstitutionService = Depends(get_institution_service)):
    return await institution_service.update_institution(institution_id, payload)

@router.delete("/{institution_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_superuser)])
async def delete_institution(institution_id: uuid.UUID, institution_service: InstitutionService = Depends(get_institution_service)):
    await institution_service.delete_institution(institution_id)
