...
### AI Assistant
**Backend:** `api/routes/assistant.py` (`POST /assistant/chat`) · **Frontend:** floating widget in `base.html` + `static/js/assistant.js`

A chat widget (bottom-right on every page once logged in, or `Ctrl/Cmd+K`) that answers "where do I..." and "find me..." questions in plain English, backed by the Claude API (`ANTHROPIC_API_KEY` / `AI_MODEL` in `backend/.env`; the widget degrades to a friendly "not configured" message if the key is blank). It has exactly two tools:
- `open_page` resolves a page name to a real URL from a fixed, role-filtered site map — it can never invent a route.
- `search_records` looks up publications, researchers, conferences, projects, or institutions by calling the *same* list/search functions those modules already expose (`list_publications`, `search_researchers`, `list_conferences`, `list_projects`, `search_institutions`), so results are automatically scoped by the asker's role with no duplicated permission logic.

Every link the assistant shows is either from that allow-list or built from a real row a tool call just returned — the model is instructed to never state a title, date, or URL it didn't just get back from a tool.
