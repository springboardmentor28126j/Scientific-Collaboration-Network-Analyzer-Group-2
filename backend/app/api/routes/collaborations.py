...
def _ordered_pair(researcher_a_id: int, researcher_b_id: int) -> tuple[int, int]:
    ...
    smaller researcher id first (see the ck_collaboration_ordered_pair
    constraint) so 'does an edge exist between A and B' is a single lookup."""
    return (
...
def _get_pending_request_between(db: Session, a_id: int, b_id: int) -> CollaborationRequest | None:
    return (
        ...
        .filter(
            ...
            or_(
                (CollaborationRequest.requester_id == a_id)
                & (CollaborationRequest.addressee_id == b_id),
                (CollaborationRequest.requester_id == b_id)
                & (CollaborationRequest.addressee_id == a_id),
            ),
        )
        .first()
    )
...
def _recompute_collaboration_metrics(db: Session, collaboration: Collaboration) -> Collaboration:
    """Finds every publication where both researchers appear as an author
    (workspace has no primary/co-author distinction -- just membership in
    publication_authors), syncs collaboration_publications to match exactly,
    and recomputes strength/first/last from that set. Idempotent."""
    ...
    existing_links = {
        link.publication_id: link for link in collaboration.shared_publications
    }

    for pub_id in shared_ids - existing_links.keys():
        db.add(
            CollaborationPublication(
                collaboration_id=collaboration.id, publication_id=pub_id
            )
        )
    ...
    if shared_ids:
        years = [
            row[0]
            for row in db.query(Publication.year)
            .filter(Publication.id.in_(shared_ids))
            .all()
...
def _collaboration_out(
    collaboration: Collaboration, viewer_researcher_id: int | None = None
) -> CollaborationOut:
    partner = None
...
def _collaboration_with_context(db: Session, collaboration_id: int) -> Collaboration | None:
    return (
        ...
        .options(
            ...
            selectinload(Collaboration.shared_publications).selectinload(
...
def send_collaboration_request(
    ...
    if payload.addressee_researcher_id == me.id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="You can't connect with yourself"
        )
    ...
    if _get_collaboration_between(db, me.id, addressee.id) is not None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="You're already collaborators"
        )
    ...
    req = CollaborationRequest(
        requester_id=me.id, addressee_id=addressee.id, message=payload.message
    )
    db.add(req)
    db.commit()
    db.refresh(req)
...
def list_collaboration_requests(
    ...
    if direction == "incoming":
        ...
        query = query.filter(
            or_(
                CollaborationRequest.addressee_id == me.id,
...
def respond_to_collaboration_request(
    ...
    if payload.status == CollaborationRequestStatus.ACCEPTED:
        create_notification(
            ...
            message=(
                f"{req.addressee.user.email if req.addressee.user else 'A researcher'} "
    ...
    log_audit(
        ...
        action=f"collaboration_request_{payload.status.value}",
        entity_type="collaboration_request",
        entity_id=req.id,
        details=f"requester_id={req.requester_id} addressee_id={req.addressee_id}",
    )
...
def collaboration_network(
    ...
    """BFS ego network outward from the current researcher, up to `depth`
    hops along Collaboration edges."""
    me = _get_current_researcher(db, current_user)
    max_nodes = 60
    ...
    for _ in range(max(1, depth)):
        ...
        rows = (
            ...
            .filter(
                or_(
                    Collaboration.researcher1_id.in_(frontier),
                    Collaboration.researcher2_id.in_(frontier),
                )
            )
            .all()
        )
    ...
    researchers = (
        db.query(Researcher)
        .options(selectinload(Researcher.user))
        .filter(Researcher.id.in_(node_ids))
        .all()
    )
...
def suggested_collaborators(
    ...
    """Ranks non-connected researchers by mutual collaborators first, then
    by sharing a department or institution with me. Workspace has no
    skills/interests tables (unlike the reference), so that half of the
    original scoring is replaced with this simpler, still-explainable
    heuristic. Excludes myself, existing collaborators, and anyone with a
    pending request (either direction) already in flight."""
    me = _get_current_researcher(db, current_user)

    existing_collab_ids = {
        row[0]
        for row in db.query(Collaboration.researcher1_id)
        .filter(Collaboration.researcher2_id == me.id)
        .all()
    } | {
        row[0]
        for row in db.query(Collaboration.researcher2_id)
        .filter(Collaboration.researcher1_id == me.id)
        .all()
    }
    pending_ids = {
        row[0]
    ...
    for collaborator_id in existing_collab_ids:
        their_partners = {
            row[0]
            for row in db.query(Collaboration.researcher1_id)
            .filter(Collaboration.researcher2_id == collaborator_id)
            .all()
        } | {
            row[0]
            for row in db.query(Collaboration.researcher2_id)
            .filter(Collaboration.researcher1_id == collaborator_id)
            .all()
        }
    ...
    if me.institution_id is not None:
        candidate_ids |= {
            ...
            for row in db.query(Researcher.id)
            .filter(
                Researcher.institution_id == me.institution_id,
                Researcher.id.notin_(excluded),
            )
            .all()
        }
    ...
    candidates = (
        db.query(Researcher)
        .options(selectinload(Researcher.user))
        .filter(Researcher.id.in_(candidate_ids))
        .all()
    )
    ...
    for candidate in candidates:
        ...
        results.append(
            SuggestedCollaboratorOut(
                researcher=_brief(candidate), reason=reason, mutual_collaborator_count=mutual
            )
        )
...
def get_collaboration(
    ...
    if not is_participant and current_user.role != UserRole.SYSTEM_ADMIN:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You can only view collaborations you're part of",
        )

    base = _collaboration_out(
        collaboration, viewer_researcher_id=me.id if is_participant else None
    )
    shared_publications = [
        SharedPublicationOut(
            publication_id=link.publication.id,
