...
from sqlalchemy import (
    CheckConstraint, Date, DateTime, Enum, ForeignKey, Integer, Text, UniqueConstraint,
)
...
class CollaborationRequest(Base):
    ...
    requester_id: Mapped[int] = mapped_column(Integer, ForeignKey("researchers.id"), nullable=False)
    addressee_id: Mapped[int] = mapped_column(Integer, ForeignKey("researchers.id"), nullable=False)
    status: Mapped[CollaborationRequestStatus] = mapped_column(
        Enum(CollaborationRequestStatus, values_callable=lambda enum_cls: [m.value for m in enum_cls]),
...
class Collaboration(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    researcher1_id: Mapped[int] = mapped_column(Integer, ForeignKey("researchers.id"), nullable=False)
    researcher2_id: Mapped[int] = mapped_column(Integer, ForeignKey("researchers.id"), nullable=False)
...
class CollaborationPublication(Base):
    ...
    collaboration: Mapped["Collaboration"] = relationship("Collaboration", back_populates="shared_publications")
