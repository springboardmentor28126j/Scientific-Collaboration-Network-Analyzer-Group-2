...
const config = {
  researcher: { title: 'Researcher Dashboard', subtitle: 'Track your scholarly activity, projects, and collaboration network.', icon: 'bi-person-workspace', actions: [['bi-journal-text','Manage my publications','publications.html'],['bi-folder2-open','Open my projects','projects.html'],['bi-people-fill','View collaboration network','network.html'],['bi-calendar-event','Browse conferences','conferences.html'],['bi-quote','View citations','citations.html'],['bi-file-earmark-bar-graph','Open research reports','report.html']] },
  institution: { title: 'Institution Dashboard', subtitle: "Monitor your institution's researchers, output, projects, and collaborations.", icon: 'bi-building', actions: [['bi-people','Manage institution researchers','researchers.html'],['bi-journal-text','Review institution publications','publications.html'],['bi-folder2-open','Monitor institution projects','projects.html'],['bi-people-fill','Open collaboration network','network.html'],['bi-clipboard-check','Assign reviews','reviews.html'],['bi-file-earmark-bar-graph','Generate institution report','report.html']] },
  publisher: { title: 'Publisher Dashboard', subtitle: 'Manage the publication pipeline, reviewer assignments, and citation records.', icon: 'bi-journal-check', actions: [['bi-journal-plus','Publication repository','publications.html'],['bi-clipboard-check','Assign reviewers','reviews.html'],['bi-quote','Citation records','citations.html'],['bi-calendar-event','Conference publications','conferences.html'],['bi-file-earmark-bar-graph','Publication analytics','report.html']] },
  reviewer: { title: 'Reviewer Dashboard', subtitle: 'Complete assigned reviews, record decisions, and check citation context.', icon: 'bi-clipboard-check', actions: [['bi-clipboard-check','Open review queue','reviews.html'],['bi-journal-text','View assigned publications','publications.html'],['bi-quote','Check citations','citations.html'],['bi-file-earmark-bar-graph','Open review analytics','report.html']] }
}[pageRole];
...
const safeJson = async (url, fallback = []) => { try { const response = await fetch(url); if (!response.ok) return fallback; return await response.json(); } catch { return fallback; } };
...
async function loadRoleDashboard() {
  ...
  try {
    ...
    const dashboardData = dashboard && typeof dashboard === 'object' && !Array.isArray(dashboard) ? dashboard : {};
    const metric = key => Number(dashboardData[key]) || 0;
    ...
    const entries = pageRole === 'researcher'
      ? [['bi-journal-text','My publications',metric('total_publications'),'publications.html'],['bi-people-fill','My collaborations',metric('total_collaborations'),'collaborations.html'],['bi-calendar-event','My conferences',metric('total_conferences'),'conferences.html'],['bi-folder2-open','My projects',metric('total_projects'),'projects.html']]
      : pageRole === 'institution'
        ? [['bi-people','Institution researchers',metric('total_researchers'),'researchers.html'],['bi-journal-text','Institution publications',metric('total_publications'),'publications.html'],['bi-folder2-open','Active projects',metric('total_projects'),'projects.html'],['bi-clipboard-check','Pending reviews',pendingReviews,'reviews.html']]
        : pageRole === 'publisher'
          ? [['bi-journal-check','Published records',published,'publications.html'],['bi-send-check','Submitted records',submitted,'publications.html'],['bi-clipboard-check','Pending reviews',pendingReviews,'reviews.html'],['bi-quote','Citation records',metric('total_citations'),'citations.html']]
